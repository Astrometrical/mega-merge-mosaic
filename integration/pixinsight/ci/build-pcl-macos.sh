...
# arm64 (default) serves Apple Silicon cores; x64 serves Intel Macs. Both
# build from the single pin in pcl-pin.env (PCL >= 2.10.3 ships native arm64
# makefiles); PixInsight >= 1.9.5 is the minimum supported core on every arch.
set -euo pipefail
...
# Each arch uses the pin's native makefiles: makefile-x64 and makefile-arm64
# both exist at 2.10.8.
...
# Per-lib makes, arch-uniform. Upstream's make-3rdparty-<arch>.sh drivers do
# exactly this (cd into each lib's macosx/g++ and make); drive the same makes
# ourselves so the script does not depend on the driver's name.
